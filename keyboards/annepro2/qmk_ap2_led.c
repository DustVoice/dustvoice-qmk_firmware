...
void annepro2LedDisable(void) { sdPut(&SD0, CMD_LED_OFF); }

void annepro2LedEnable(void) { sdPut(&SD0, CMD_LED_ON); }
...
void annepro2LedNextProfile() { sdPut(&SD0, CMD_LED_NEXT_PROFILE); }
...
void annepro2LedSetHSV(uint8_t hue, uint8_t sat, uint8_t val) {
    sdPut(&SD0, CMD_LED_SET_HSV);
    sdPut(&SD0, hue);
    sdPut(&SD0, sat);
    sdPut(&SD0, val);
}
